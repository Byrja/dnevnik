...
class CallbackRoutesTests(unittest.TestCase):
    ...
    def test_admin_ab_command_registered(self) -> None:
        ...
        self.assertIn('CallbackQueryHandler(admin_export_action, pattern=r"^adminexport:[0-9]+:(txt|json)$")', src)
    ...
    def test_global_menu_text_handler_registered(self) -> None:
        src = main_py()
        self.assertIn('MessageHandler(filters.Regex(r"(?i)^\\s*в\\s*меню\\s*$"), go_menu_and_end)', src)
